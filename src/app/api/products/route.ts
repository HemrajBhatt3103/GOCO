...
export async function GET(req: NextRequest) {
  try {
    ...
    const pageSize = parseInt(searchParams.get("pageSize") ?? "20");
      ...
      // storeId param: if provided, filter for that store (public catalog)
    // if authenticated admin and no storeId param, filter by their own store
  ...
  try {
    ...
    const product = await prisma.product.create({
      data: {
        ...
        storeId: user.role === "ADMIN" ? user.id : null,
